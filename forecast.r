...
nsw_dt[,`:=`(lower=NA,upper=NA)]
nsw_fc <- data.table(date=seq(covid_dt[date==max(date)]$date+1,by="day",length.out=h),observed=NA,logistic=ystr_f,exponential=yexp_f,lower=ystr_ci$lower,upper=ystr_ci$upper)
